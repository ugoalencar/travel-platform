...
| Sale | PR READY | `gh pr list` shows **PR #13 "feat: add Sale vertical", branch `feature/sale-vertical`, state OPEN**, not merged. `services/api/src/sales.ts` does not exist on `origin/main` (confirmed: `git show origin/main:services/api/src/sales.ts` fails) but exists on `feature/sale-vertical` (283 lines). |
| Commercial Cockpit | LOCAL IMPLEMENTED | Branch `feature/commercial-cockpit` exists **only in the local git object store of this working environment — it is not on `origin` (`git branch -r` does not list it) and has no PR**. Contains real code: `services/api/src/commercial-cockpit.ts` (1211 lines), `services/api/src/commercial-queries.ts` (219 lines), `services/api/src/commercial-cockpit-parsers.ts`, migrations `007_commercial_cockpit.sql`, `008_configurable_pipelines.sql`, frontend pages under `apps/customer/src/pages/commercial/`, and a real-Postgres security test suite (`commercial-cockpit-security.test.ts`, 1099 lines). Not pushed, not opened as a PR — do not describe as "PR ready." |
| Configurable Pipeline | LOCAL IMPLEMENTED | Same branch `feature/commercial-cockpit`, commits `4bf6372`/`f6dbba8` (backend + frontend). Same caveat: local-only, unpushed, no PR. |
