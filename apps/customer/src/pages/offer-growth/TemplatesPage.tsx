...
export function TemplatesPage() {
  ...
  function handleCreate() {
    const name = newName.trim() || `Novo template ${templates.length + 1}`;
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Templates de criativo</h1>
        <p className="mt-1 text-sm text-slate-500">
          Modelos reutilizáveis de carrossel/post que alimentam a Creative Studio. Guardados
        ...
        {templates.map((template) => (
          <article key={template.id} className="rounded-lg border border-slate-200 bg-white p-4">
            ...
            <p className="text-xs text-slate-500">
              {template.pages.length} slide(s) · {template.bindings.length} vínculo(s)
            </p>
