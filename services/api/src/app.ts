...
export function buildApp(options: BuildAppOptions): FastifyInstance {
  ...
  // ============================================================
  // PARTNER CAMPAIGNS (Agent 10)
  // External-partner advertising campaigns -- placements across staff
  // dashboard, proposal, trip, customer portal, catalog. Distinct from the
  // internal "offer growth" /campaigns routes above (different table,
  // different module). RBAC floor: read = VIEWER+, create/write = AGENT+,
  // status transitions (activate/pause/complete) = MANAGER+.
  // ============================================================

  app.get('/partner-campaign-partners', { preHandler: protectedHooks }, async () => {
    requireRole(UserRole.VIEWER);
    const partners = await options.database.withTenantTransaction((client) =>
  ...
  app.post('/partner-campaign-partners', { preHandler: protectedHooks }, async (request, reply) => {
    ...
    const partner = await options.database.withTenantTransaction((client) =>
  ...
  app.get('/partner-campaigns', { preHandler: protectedHooks }, async () => {
    requireRole(UserRole.VIEWER);
    const campaigns = await options.database.withTenantTransaction((client) =>
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request) => {
      requireRole(UserRole.VIEWER);
      const campaign = await options.database.withTenantTransaction((client) =>
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request) => {
      requireRole(UserRole.VIEWER);
      const products = await options.database.withTenantTransaction((client) =>
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request) => {
      requireRole(UserRole.VIEWER);
      const placements = await options.database.withTenantTransaction((client) =>
  ...
  app.post<{ Params: { id: string }; Body: { location: string } }>(
    ...
    async (request, reply) => {
      ...
      const placement = await options.database.withTenantTransaction((client) =>
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request) => {
      requireRole(UserRole.VIEWER);
      const summary = await options.database.withTenantTransaction((client) =>
