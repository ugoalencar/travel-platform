...
const projectName = 'travel-platform-suppliers-http-postgres';
...
describe.sequential('Supplier HTTP routes', () => {
  let adminPool: Pool;
  ...
  describe('GET /transport/suppliers', () => {
    it('returns 401 without auth', async () => {
      const app = buildTestApp(runtimePool);
      const response = await app.inject({ method: 'GET', url: '/transport/suppliers' });
      expect(response.statusCode).toBe(401);
      await app.close();
    });

    it('returns only Agency A suppliers for Agency A', async () => {
      ...
      const response = await app.inject({
        method: 'GET',
        url: '/transport/suppliers',
        headers: { 'x-test-principal': 'owner' },
      });

      expect(response.statusCode).toBe(200);
      const body = response.json<{ suppliers: Array<{ agencyId: string }> }>();
      expect(body.suppliers).toHaveLength(1);
      expect(body.suppliers[0]?.agencyId).toBe(agencyAId);

      await app.close();
    });
  });

  describe('GET /transport/suppliers/:id', () => {
    it('never returns another tenant supplier (404, no leak)', async () => {
      const bId = await seedSupplier(agencyBId, 'Supplier B');

      const app = buildTestApp(runtimePool);
      const response = await app.inject({
        method: 'GET',
        url: `/transport/suppliers/${bId}`,
        headers: { 'x-test-principal': 'owner' },
      });

      expect(response.statusCode).toBe(404);
      expect(response.json()).toEqual({ error: 'Supplier not found', code: 'NOT_FOUND' });

      await app.close();
    });
  });

  describe('POST /transport/suppliers', () => {
    it('creates a supplier with default active=true', async () => {
      const app = buildTestApp(runtimePool);
      const response = await app.inject({
        method: 'POST',
        url: '/transport/suppliers',
        headers: { 'x-test-principal': 'manager' },
        payload: { name: 'Supplier A' },
      });

      expect(response.statusCode).toBe(201);
      const body = response.json<{ supplier: { agencyId: string; active: boolean } }>();
      expect(body.supplier.agencyId).toBe(agencyAId);
    ...
    it('rejects unknown field with 400', async () => {
      ...
      const response = await app.inject({
        ...
        url: '/transport/suppliers',
        headers: { 'x-test-principal': 'manager' },
        payload: { name: 'Supplier A', notARealField: 'x' },
      });
  ...
  describe('PATCH /transport/suppliers/:id', () => {
    it('never mutates another tenant supplier (404, no-op)', async () => {
      const bId = await seedSupplier(agencyBId, 'Supplier B');
  ...
  describe('RBAC', () => {
    ...
    it('lets Agency B OWNER read only Agency B data, never Agency A', async () => {
      const aId = await seedSupplier(agencyAId, 'Supplier A');
      const app = buildTestApp(runtimePool);

      const get = await app.inject({
        method: 'GET',
        url: `/transport/suppliers/${aId}`,
...
function assertSafeTestDatabase(): void {
  if (!['127.0.0.1', 'localhost'].includes(databaseHost)) {
    throw new Error('Supplier route tests require localhost only.');
  }
  if (!Number.isInteger(databasePort) || databasePort < 1024 || databasePort > 65535) {
    throw new Error('Supplier route tests require a safe local database test port.');
  }
  if (!databaseName.includes('test')) {
    throw new Error('Supplier route tests require a database name with a test marker.');
  }
  if (process.env.DATABASE_URL) {
    ...
    if (!safeHost || !safeDatabase || !safePort) {
      throw new Error('Refusing to run supplier route tests against unsafe DATABASE_URL.');
    }
  }
}
...
async function seedAgenciesAndUsers(pool: Pool): Promise<void> {
  await pool.query(
    `
      ...
      VALUES
        ($1, 'Agency A', 'agency-a-suppliers-http-test', 'agency-a@example.test', 'FREE', 'ACTIVE'),
        ($2, 'Agency B', 'agency-b-suppliers-http-test', 'agency-b@example.test', 'FREE', 'ACTIVE');
    `,
    [agencyAId, agencyBId],
  );
  await pool.query(
    `
      ...
      VALUES
        ($1, $2, 'user-a@example.test', 'User A', 'ADMIN', 'hash-for-suppliers-http-test-only', 'ACTIVE'),
        ($3, $4, 'user-b@example.test', 'User B', 'ADMIN', 'hash-for-suppliers-http-test-only', 'ACTIVE');
    `,
