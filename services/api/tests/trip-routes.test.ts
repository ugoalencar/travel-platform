...
// listTrips()'s category computation (this session's work) does an EXISTS
// against excursion_customers/air_services/land_services regardless of
// whether any row matches -- all three tables need to exist for the query
// to compile, so every route touching GET /trips 500ed without them.
// air_services/land_services (039/040) need suppliers (038) and bookings
// (005), which pull in the sequential 003-018 chain (including
// FinancialObligationStatus from 010) -- reusing the full 001-024 block
// (proven working -- the same list financial-http.test.ts/partners.test.ts
// use) rather than hand-picking dependencies one at a time again (found
// via two rounds of real CI failures: "relation excursion_customers does
// not exist", then "type FinancialObligationStatus does not exist").
const migrations = [
  ...
  // createCustomer() always populates -- omitting it made every customer
  // INSERT fail with "column protocol_number does not exist" (found via
  // a real CI run).
...
describe('Trip HTTP routes (Package 1)', () => {
  let adminPool: Pool;
