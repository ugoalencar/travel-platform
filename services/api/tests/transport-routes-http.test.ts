...
const projectName = 'travel-platform-transport-routes-http-postgres';
...
describe.sequential('Route HTTP routes', () => {
  let adminPool: Pool;
  ...
  describe('GET /transport/routes', () => {
    it('returns 401 without auth', async () => {
      ...
      const response = await app.inject({ method: 'GET', url: '/transport/routes' });
      expect(response.statusCode).toBe(401);
      await app.close();
    });

    it('returns only Agency A routes for Agency A', async () => {
      ...
      const body = response.json<{ routes: Array<{ agencyId: string }> }>();
      expect(body.routes).toHaveLength(1);
      expect(body.routes[0]?.agencyId).toBe(agencyAId);
  ...
  describe('GET /transport/routes/:id', () => {
    it('never returns another tenant route (404, no leak)', async () => {
      const bId = await seedRoute(agencyBId, 'B1', 'B2');

      const app = buildTestApp(runtimePool);
      const response = await app.inject({
        method: 'GET',
        url: `/transport/routes/${bId}`,
        headers: { 'x-test-principal': 'a' },
      });

      expect(response.statusCode).toBe(404);
      expect(response.json()).toEqual({ error: 'Route not found', code: 'NOT_FOUND' });
  ...
  describe('POST /transport/routes', () => {
    it('creates a route with default active=true', async () => {
      const app = buildTestApp(runtimePool);
      const response = await app.inject({
        method: 'POST',
        url: '/transport/routes',
      ...
      const body = response.json<{ route: { agencyId: string; active: boolean } }>();
      expect(body.route.agencyId).toBe(agencyAId);
      expect(body.route.active).toBe(true);
    ...
    it.each(['agencyId', 'tenantId', 'id', 'createdAt', 'updatedAt'] as const)(
      ...
      async (field) => {
        ...
        const response = await app.inject({
          ...
          url: '/transport/routes',
          headers: { 'x-test-principal': 'manager' },
          payload: { origin: 'A', destination: 'B', [field]: 'x' },
        });
    ...
    it('rejects unknown field with 400', async () => {
      ...
      const response = await app.inject({
        ...
        url: '/transport/routes',
        headers: { 'x-test-principal': 'manager' },
        payload: { origin: 'A', destination: 'B', notARealField: 'x' },
      });
    ...
    it('rejects negative estimatedDuration/distance with 400', async () => {
      const app = buildTestApp(runtimePool);
      const response = await app.inject({
        method: 'POST',
        url: '/transport/routes',
        headers: { 'x-test-principal': 'manager' },
        payload: { origin: 'A', destination: 'B', estimatedDuration: -1 },
      });
    ...
    it('rejects missing origin/destination with 400', async () => {
      const app = buildTestApp(runtimePool);
      const response = await app.inject({
        method: 'POST',
        url: '/transport/routes',
  ...
  describe('PATCH /transport/routes/:id', () => {
    it('never mutates another tenant route (404, no-op)', async () => {
      const bId = await seedRoute(agencyBId, 'B1', 'B2');
  ...
  describe('RBAC', () => {
    ...
    it('keeps a high role from Agency A blocked from Agency B routes', async () => {
      const bId = await seedRoute(agencyBId, 'B1', 'B2');
      const app = buildTestApp(runtimePool);
    ...
    it('lets Agency B OWNER read only Agency B data, never Agency A', async () => {
      const aId = await seedRoute(agencyAId, 'A1', 'A2');
      const app = buildTestApp(runtimePool);

      const get = await app.inject({
        method: 'GET',
        url: `/transport/routes/${aId}`,
...
function assertSafeTestDatabase(): void {
  if (!['127.0.0.1', 'localhost'].includes(databaseHost)) {
    throw new Error('Route route tests require localhost only.');
  }
  if (!Number.isInteger(databasePort) || databasePort < 1024 || databasePort > 65535) {
    throw new Error('Route route tests require a safe local database test port.');
  }
  if (!databaseName.includes('test')) {
    throw new Error('Route route tests require a database name with a test marker.');
  }
  if (process.env.DATABASE_URL) {
    ...
    if (!safeHost || !safeDatabase || !safePort) {
      throw new Error('Refusing to run route route tests against unsafe DATABASE_URL.');
    }
  }
}
...
async function seedAgenciesAndUsers(pool: Pool): Promise<void> {
  await pool.query(
    `
      ...
      VALUES
        ($1, 'Agency A', 'agency-a-transport-routes-http-test', 'agency-a@example.test', 'FREE', 'ACTIVE'),
        ($2, 'Agency B', 'agency-b-transport-routes-http-test', 'agency-b@example.test', 'FREE', 'ACTIVE');
    `,
    [agencyAId, agencyBId],
  );
  await pool.query(
    `
      ...
      VALUES
        ($1, $2, 'user-a@example.test', 'User A', 'ADMIN', 'hash-for-transport-routes-http-test-only', 'ACTIVE'),
        ($3, $4, 'user-b@example.test', 'User B', 'ADMIN', 'hash-for-transport-routes-http-test-only', 'ACTIVE');
    `,
