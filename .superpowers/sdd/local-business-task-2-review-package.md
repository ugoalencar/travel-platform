...
## Git Status
```
 M scripts/seed-tenant-demo-data.cjs
?? .superpowers/sdd/local-business-task-2-report.md
?? scripts/demo-business-stories.cjs
```

## Tracked Diff: scripts/seed-tenant-demo-data.cjs
```diff
diff --git a/scripts/seed-tenant-demo-data.cjs b/scripts/seed-tenant-demo-data.cjs
index 2c70dab..d80721f 100644
--- a/scripts/seed-tenant-demo-data.cjs
+++ b/scripts/seed-tenant-demo-data.cjs
@@ -15,6 +15,7 @@
...
@@ -81,6 +82,9 @@ async function seedTenantData() {
...
## New File: scripts/demo-business-stories.cjs
```javascript
const STORY_IDS = {
  marianaCancun: {
  ...
  for (const [id, revenueSaleId, description, amount, dueDate, status] of rows) {
    await pool.query(
      `INSERT INTO revenues
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'BRL', $8, $8, CASE WHEN $9 = 'PAID' THEN $8::date ELSE NULL END, 'PIX', $9, 'Demo installment schedule for Mariana / Cancun sale', now(), now())
       ON CONFLICT (agency_id, id) DO UPDATE SET
